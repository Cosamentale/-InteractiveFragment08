...
const displayShaderSource = `
    ...
    uv.x *=fac;
    float my = length(mouse.y-0.5);
    float dm = my*7.;
    vec2 ug  = uv*mix(7.,0.1,my);
        float d1 = 1.;

        float tb =1.57*(mouse.x-0.5);
        float va = 0.05;float vb = 0.1;
        float a = 0.;
        float ti = 3.;
        ...
        ug *= rot(tb*a);
        ug2 *= rot(tb*(a+1.));
        float l1 = length(fract(ug.x)-0.5);
        float l2 = length(fract(ug.y)-0.5);
        float l3 = length(fract(ug2.x)-0.5);
        float l4 = length(fract(ug2.y)-0.5);
        float f1 = min(smoothstep(va,vb,min(l1,l3)),smoothstep(va,vb,min(l2,l4)));
        d1 =mix(f1,d1,f1);
        }

    gl_FragColor = vec4(smoothstep(0.4,0.,d1));
    }
`;
...
function drawDisplay (target) {
 ...
 gl.uniform1f(displayMaterial.uniforms.resolution, canvas.width/canvas.height);
  gl.uniform2f(displayMaterial.uniforms.mouse, pointers[0].texcoordX, pointers[0].texcoordY);
  gl.uniform1i(displayMaterial.uniforms.nbr, 3);
    blit(target);
}
